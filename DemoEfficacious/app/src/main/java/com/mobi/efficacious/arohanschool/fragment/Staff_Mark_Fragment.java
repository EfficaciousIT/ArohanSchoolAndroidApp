...
import com.mobi.efficacious.arohanschool.adapters.Staff_Mark_Adapter;
import com.mobi.efficacious.arohanschool.adapters.Teacher_Mark_Adapter;
...
public class Staff_Mark_Fragment extends Fragment {
    View myview;
...
//                            tv_dateSelection.setText(dayOfMonth + "-" + (monthOfYear + 1) + "-" + year);
    ...
    public void AttendanceMark() {
        ...
        try
        {
//            final ArrayList<AttendanceDetail> teacherlist=((Staff_Mark_Adapter)madapter).getTchrList();
...
//                    MarkAttendence attendanceDetail = new MarkAttendence(4,intSchoolId,intAcademicId,Selected_Date,status,1,FCMToken);
                      MarkAttendence attendanceDetail = new MarkAttendence(4,intSchoolId,intAcademicId,Selected_Date,status,intTeacher_id,FCMToken);
                    ...
                    call.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Observer<ResponseBody>() {
                        ...
                        public void onError(Throwable t) {
                            progress.dismiss();
                            Toast.makeText(getActivity(), "Response taking time seems Network issue onError AttendanceMark!", Toast.LENGTH_SHORT).show();
    ...
    public void LoginAsync() {
        try {
            ...
            {
                command="BackDateStaff";
            }else
            {
                command= "BackDateStaff";
            }
//            mCompositeDisposable.add(service.getAttendanceDetails(command,Schooli_id,"4","","",academic_id,Selected_Date,"","1")

            mCompositeDisposable.add(service.getAttendanceDetails(command,Schooli_id,"4","","",academic_id,Selected_Date,"",String.valueOf(intTeacher_id))
    ...
    public DisposableObserver<AttendanceDetailPojo> getObserver(){
        return new DisposableObserver<AttendanceDetailPojo>() {
            ...
            public void onError(@NonNull Throwable e) {
                progress.dismiss();
                Toast.makeText(getActivity(), "Response taking time seems Network issue! in Error", Toast.LENGTH_SHORT).show();
            }
