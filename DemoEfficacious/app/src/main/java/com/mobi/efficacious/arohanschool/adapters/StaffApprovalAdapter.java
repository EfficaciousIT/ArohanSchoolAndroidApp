...
import com.mobi.efficacious.arohanschool.dialogbox.image_zoom_dialog;
...
public class StaffApprovalAdapter  extends RecyclerView.Adapter<StaffApprovalAdapter.LeaveListHolder> {
    ...
    String LeaveApplication_id, intStaff_id;
    private ProgressDialog progress;
    public StaffApprovalAdapter(ArrayList<LeaveDetail> dataList,Context context) {
    ...
    public void onBindViewHolder(final LeaveListHolder holder, final int position) {
        ...
        holder.approval_checkbx.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                try {
                    intStaff_id = String.valueOf(itemsArrayList.get(position).getIntStaffid());
                    int_Approval = "1";
                } catch (Exception ex) {
    ...
    public void  LeaveCountAsync (){
        try {
            ...
            String command;
            if (LeaveType.contentEquals("CL")) {
                command="UpdateCLCountStaff";
            } else {
                command="UpdateMLCountStaff";
            }
            DataService service = RetrofitInstance.getRetrofitInstance().create(DataService.class);
            LeaveDetail leaveDetail = new LeaveDetail(TotalLeaveCOunt,TotalLeaveCOunt,Integer.parseInt(intStaff_id));
